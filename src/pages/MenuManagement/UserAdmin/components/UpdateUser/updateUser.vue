        ...
        <div class="dialog-body">
            ...
            <t-form ref="formRef" :data="formData" :rules="rules">
                <t-form-item :label="$t('userDialog.account')" name="account" :rules="[{ required: true, message: $t('userDialog.accountRequired') }]">
                    <t-input v-model="formData.account" :placeholder="$t('userDialog.accountPlaceholder')" :disabled="formData.account === 'admin' ? true : false" />
                </t-form-item>
...
<script lang="ts" setup>
    ...
    const handleUpdateUser = async () => { 
       ...
       if (form_valid !== true) {
        ...
        api.updatePersonalInfo(props.userId,formData.value)
            .then((res) => { 
                if (res.code === 2000) { 
                    visible.value = false;
                    emit('resetUserList')
                    MessagePlugin.success(t('userDialog.updateSuccess'));
                    handleReset()
                }
            })
    }
    ...
    const getUserInfo = async () => { 
        const res = await api.getPersonalInfo(props.userId)
        .then(res => { 
            if (res.code === 2000) { 
